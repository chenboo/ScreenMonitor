...
//计算出nStartLine这行的像素数据变化的rect区域(会适当的扩大范围DEF_STEP,OFF_SET),然后把内容复制到m_rectBuffer里
bool CScreenSpy::ScanChangedRect(int nStartLine)
{
	...
	::SetRect(&m_changeRect, -1, nStartLine - DEF_STEP, -1, nStartLine + DEF_STEP * 2);
	 ...
	 // m_nIncSize 意思是 每次比较的四个字节内容(DWORD)实际是几个像素？比如如果是8位色，就是1个像素1个字节表示，每次我们比较 就相当于比较了4个像素的内容，如果是16位 就相当于比较2个像素的值，所以构造函数会用32除位数
